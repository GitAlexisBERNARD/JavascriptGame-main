...
let jaugeValue = 15; // Valeur actuelle de la jauge
...
function generateRandomEnemy() {
  ...
  spawnEnemy(randomType, randomDirection);
}
...
function spawnInitialEnemies() {
  for (let i = 0; i < 3; i++) { // Réduire le nombre d'ennemis initiaux
    setTimeout(() => {
      generateRandomEnemy();
    }, i * 4000); // Augmenter l'intervalle entre chaque ennemi initial
  }
}
function continueSpawningEnemies() {
  let spawnInterval = 4000; // Définir un intervalle initial plus long

  setInterval(() => {
    generateRandomEnemy();

    // Optionnel : Réduire progressivement l'intervalle pour augmenter la difficulté
    spawnInterval = Math.max(2000, spawnInterval - 100); // Réduire l'intervalle à chaque fois, sans aller en dessous de 2000 ms
  }, spawnInterval);
}
